...
function teams(){e.noTeams.hidden=state.teams.length>0;e.teams.innerHTML=state.teams.map(t=>{let ps=(t.playerIds||[]).map(id=>state.members.find(m=>m.id===id)).filter(Boolean),present=new Set(ps.map(p=>p.class)),missing=CLASSES.filter(c=>!present.has(c)),total=ps.reduce((s,p)=>s+Number(p.power),0);return `<article class="teamcard"><div class="teamtop"><h3>Team ${t.number}</h3><b>${power(total)}</b></div><div class="meta"><span>${ps.length}/4 players</span>${CLASSES.filter(c=>present.has(c)).map(c=>classBadge(c)).join('')}</div><ul class="players">${ps.map(p=>`<li><div><b>${esc(p.name)}</b><small>${esc(p.class)}</small></div><b>${power(p.power)}</b></li>`).join('')}</ul>${missing.length?`<div class="missing">Missing: ${missing.join(', ')}</div>`:'<div class="present">All 4 classes present</div>'}</article>`}).join('')}
